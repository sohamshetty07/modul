...
import { FileText, Download, Trash2, Layers, MoveUp, MoveDown, GripVertical, Pencil, Image as ImageIcon, Minimize2, Scissors, Check, X } from "lucide-react";
...
export default function PDFTools() {
  ...
  const [compressionQuality, setCompressionQuality] = useState(70); // 0-100

  // Splitter State
  const [splitFile, setSplitFile] = useState<File | null>(null);
  const [splitPages, setSplitPages] = useState<PDFPage[]>([]); // New state for visual pages
  ...
  const generateThumbnails = useCallback(async (file: File) => {
    ...
    try {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            // Render at a low scale for fast thumbnails
            ...
            pageList.push({ index: i - 1, url: thumbUrl, selected: true }); // Default to selected
        }
  ...
  // --- HELPER: Load PDF.js Dynamically (CRASH PROOF FIX) ---
  ...
  const handleToolChange = (v: string) => {
      ...
      setFiles([]); 
      setSplitFile(null);
      setSplitPages([]); // Clear split pages on tab change
  };
  ...
  const handleCompress = async () => {
      ...
      try {
          ...
          if (pdfBlob.size > originalSize) {
              ...
              toast({ title: "Compression Skipped", description: `Output (${formatBytes(pdfBlob.size)}) was larger than original.`, className: "bg-orange-600 text-white border-none" });
          } else {
              downloadBlob(pdfBlob, `${outputName}-compressed.pdf`, "application/pdf");
              toast({ title: "Compression Complete!", description: `Saved ${formatBytes(originalSize - pdfBlob.size)} of space.`, className: "bg-green-600 text-white border-none" });
          }
  ...
  // --- ACTION 4: SPLIT (Uses Thumbnails) ---
  const handleSplit = async () => {
      ...
      try {
          ...
          // Use the page indices from the selected thumbnails
          const pageIndicesToKeep = selectedPages.map(p => p.index); // 0-indexed
      ...
      <Tabs defaultValue="merge" value={activeTool} onValueChange={handleToolChange} className="w-full">
            <div className="flex justify-center mb-8">
                <TabsList className="bg-slate-900 border border-slate-800 h-10 p-1">
                    <TabsTrigger value="merge" className="px-6 data-[state=active]:bg-red-500/20 data-[state=active]:text-red-400">Merge</TabsTrigger>
                    <TabsTrigger value="split" className="px-6 data-[state=active]:bg-red-500/20 data-[state=active]:text-red-400">Split</TabsTrigger>
                    <TabsTrigger value="compress" className="px-6 data-[state=active]:bg-blue-500/20 data-[state=active]:text-blue-400">Compress</TabsTrigger>
                    <TabsTrigger value="images" className="px-6 data-[state=active]:bg-yellow-500/20 data-[state=active]:text-yellow-400">To Images</TabsTrigger>
                </TabsList>
            </div>
            ...
            <div className="space-y-6">
                 ...
                 {/* 1. DROPZONE */}
                 {(files.length === 0 && activeTool !== 'split') || (!splitFile && activeTool === 'split') ? (
                     <div 
                        {...getRootProps()} 
                        ...
                        <div className="space-y-4">
                             ...
                             <div>
                                <p className="text-lg font-medium text-slate-200">
                                    {activeTool === 'merge' ? "Drop PDFs to combine" : activeTool === 'split' ? "Drop ONE PDF to split" : "Drop a PDF to process"}
                                </p>
                     ...
                     <div className="space-y-4">
                         ...
                         {/* Thumbnail View for Splitter */}
                         {activeTool === 'split' && splitFile && (
                            <div className="flex flex-col gap-4">
                                {/* Header and Select All/None Buttons */}
                                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-3 rounded-xl">
                                    <p className="text-sm text-slate-300 font-medium truncate">{splitFile.name} ({splitPages.length} Pages)</p>
                                    <div className="flex gap-2">
                                        <Button onClick={selectAll} size="sm" variant="outline" className="h-7 text-xs border-slate-700 text-green-400 hover:text-green-300">Select All</Button>
                                        <Button onClick={selectNone} size="sm" variant="outline" className="h-7 text-xs border-slate-700 text-red-400 hover:text-red-300">Select None</Button>
                                        <Button size="icon" variant="ghost" className="h-7 w-7 text-slate-500 hover:text-red-400" onClick={() => handleToolChange('split')}>
                                            <Trash2 size={16} />
                                        </Button>
                                    </div>
                                </div>
                         ...
                         {/* List View for Merge/Compress/Images */}
                         {activeTool !== 'split' && files.map((file, i) => (
                             <div key={i} className="flex items-center gap-4 p-3 bg-slate-900 border border-slate-800 rounded-xl group hover:border-slate-700 transition-all">
                                 ...
                                 <div className="w-10 h-10 bg-red-500/10 rounded-lg flex items-center justify-center text-red-500 font-bold text-xs">
                                     PDF
                                 </div>
                                 ...
                                 {activeTool === 'merge' && (
                                     <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                         <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-white" onClick={() => moveFile(i, -1)}>
                                             <MoveUp size={14} />
                                         </Button>
                                         <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-white" onClick={() => moveFile(i, 1)}>
                                             <MoveDown size={14} />
                 ...
                 {/* 3. SETTINGS & ACTIONS */}
                 {((files.length > 0 && activeTool !== 'split') || splitFile) && (
                         ...
                         <div className="flex flex-col sm:flex-row gap-4 items-end">
                             ...
                             <Button 
                                ...
                                className={`h-10 px-8 font-medium min-w-[140px]
                                    ${activeTool === 'merge' || activeTool === 'split' ? "bg-red-600 hover:bg-red-500" : 
                                      activeTool === 'compress' ? "bg-blue-600 hover:bg-blue-500" : 
                             ...
                             >
                                 {isProcessing ? (
                                     ...
                                     activeTool === 'split' ? `Split ${splitPages.filter(p => p.selected).length} Pages` :
                                     activeTool === 'compress' ? "Compress PDF" : "Convert to Images"
                                 )}
                             </Button>
                         </div>
                     </div>
